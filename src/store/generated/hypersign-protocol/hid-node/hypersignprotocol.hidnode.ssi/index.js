        ...
        async QuerySchemaParam({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.next_key != null) {
                    let next_values = (await queryClient.querySchemaParam({ ...query, 'pagination.key': value.pagination.next_key })).data;
                    value = mergeResults(value, next_values);
                }
                commit('QUERY', { query: 'SchemaParam', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QuerySchemaParam', payload: { options: { all }, params: { ...key }, query } });
